...
###############################################################################
def get_head_rotations_df_from_keypoints(
    df,
    earL_kp=1,    # 'EarL' keypoint index
    earR_kp=2,    # 'EarR' keypoint index
    snout_kp=3,   # 'Snout' keypoint index
    r_root_inv=None,
    r_root_inv_oriented=None
):
    """
    Compute global head rotation and head rotation in body coordinates from raw keypoints.
    """
    print("Processing head rotations from keypoints...")
    ...
    earL_x, earL_y, earL_z = df[f"kp{earL_kp}_x"], df[f"kp{earL_kp}_y"], df[f"kp{earL_kp}_z"]
    earR_x, earR_y, earR_z = df[f"kp{earR_kp}_x"], df[f"kp{earR_kp}_y"], df[f"kp{earR_kp}_z"]
    snout_x, snout_y, snout_z = df[f"kp{snout_kp}_x"], df[f"kp{snout_kp}_y"], df[f"kp{snout_kp}_z"]

    for t in range(nf):
        if np.isnan(earL_x[t]) or np.isnan(earR_x[t]) or np.isnan(snout_x[t]):
            continue

        # Step 1: Construct the `head_x` vector (left to right direction)
        earL = np.array([earL_x[t], earL_y[t], earL_z[t]], dtype=float)
        earR = np.array([earR_x[t], earR_y[t], earR_z[t]], dtype=float)
        head_x = earR - earL

        # Normalize `head_x`
        if np.linalg.norm(head_x) < 1e-7:
            continue
        head_x /= np.linalg.norm(head_x)

        # Step 2: Construct the `head_z` vector (front direction)
        snout = np.array([snout_x[t], snout_y[t], snout_z[t]], dtype=float)
        head_center = (earL + earR) / 2  # Midpoint between the ears
        head_z = snout - head_center

        # Normalize `head_z`
        if np.linalg.norm(head_z) < 1e-7:
            continue
        head_z /= np.linalg.norm(head_z)

        # Step 3: Construct the `head_y` vector (up direction) using the cross-product
        head_y = np.cross(head_z, head_x)

        # Step 4: Assemble the rotation matrix
        mat_inv = np.vstack([head_x, head_y, head_z])
        global_head_rm[t, :, :] = mat_inv

        # Optional: Calculate head orientation in the body frame if needed
        if r_root_inv is not None:
            rhx = r_root_inv[t, :, :] @ head_x
            rhy = r_root_inv[t, :, :] @ head_y
            rhz = r_root_inv[t, :, :] @ head_z
            r_heads[t, :, :] = np.vstack([rhx, rhy, rhz])

        if r_root_inv_oriented is not None:
            rhx2 = r_root_inv_oriented[t, :, :] @ head_x
            rhy2 = r_root_inv_oriented[t, :, :] @ head_y
            rhz2 = r_root_inv_oriented[t, :, :] @ head_z
...
def process_tracking_data_df(
    ...
    # 2) Head rotations
    global_head_rm, r_heads, body_turned, head_ups = get_head_rotations_df_from_keypoints(
        df,         
        earL_kp,  # Example: keypoint 1 is 'EarL'
        earR_kp,  # Example: keypoint 2 is 'EarR'
        snout_kp,  # Example: keypoint 3 is 'Snout'
