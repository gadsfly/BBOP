...
# assert angles.shape[0] == ego_pose.shape[0]

# # Combine features
# combined_features = np.hstack([ angles, ego_pose])

# # Concatenate labels
# combined_labels = angle_labels + labels

# Perform PCA on the combined features
pca_features, pca_labels = features.pca(
    features=combined_features,
    labels=combined_labels,
    categories=["vel", "ang", "ego_euc"], #["vel", "ego_euc", "ang", "avel"], , "head_ang", "eluer_ang", "avel"
    n_pcs=8,
    downsample=1,
    method="fbpca",
)
...
pc_wlet, pc_wlet_labels = features.pca(
    ...
    categories=["wlet_vel","wlet_ang", "wlet_ego_euc"],
    n_pcs=8,
    method="fbpca",
)
