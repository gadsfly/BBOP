import os
import subprocess
import itertools
import csv
from datetime import datetime
import argparse

def load_completed_combinations(log_file):
    """
    从日志文件中读取已经运行过的 parameter 组合，并返回一个集合。
    如果日志文件不存在或为空，则返回空集合。
    """
    completed = set()
    if os.path.exists(log_file) and os.path.getsize(log_file) > 0:
        with open(log_file, newline="") as csvfile:
            reader = csv.DictReader(csvfile)
            for row in reader:
                combo = row.get("combination")
                if combo:
                    completed.add(combo)
    return completed

def ensure_log_header(log_file):
    """
    如果日志文件不存在或文件为空，则以追加模式写入表头；否则什么都不做，避免覆盖。
    """
    header = [
        "timestamp",
        "combination",
        "wnd_size",
        "stp_size",
        "max_wnd",
        "diff_thres",
        "pnr_thresh",
        "result_file"
    ]
    if not os.path.exists(log_file) or os.path.getsize(log_file) == 0:
        with open(log_file, "a", newline="") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(header)

def main(session_dir, animal, dry_run=False):
    """
    session_dir: path to the folder where data/analysis should be done
    animal:      an identifier for the subject/animal
    dry_run:     True 时只打印哪些组合会被跳过或执行，不真正跑 minian 脚本，也不写日志
    """

    # === 参数网格设置 ===
    wnd_size_vals    = [700, 1000, 1500]
    stp_size_vals    = [700]
    diff_thres_vals  = [3.5, 4.0, 5.0]
    pnr_thresh_vals  = [1.1, "auto"]

    if "pmc" in session_dir.lower():
        max_wnd_vals = [15]
    elif "v1" in session_dir.lower():
        max_wnd_vals = [25]
    else:
        max_wnd_vals = [25]

    # === 日志文件准备 ===
    log_file = os.path.join(session_dir, "param_search_log.csv")
    ensure_log_header(log_file)
    completed_combinations = load_completed_combinations(log_file)

    # === 参数模板 ===
    param_template = r"""
import os
import numpy as np
import pathlib

param_seeds_init = {{
    'wnd_size': {wnd_size},  
    'method': 'rolling',
    'stp_size': {stp_size},  
    'max_wnd': {max_wnd},    
    'diff_thres': {diff_thres}  
}}

param_denoise = {{"method": "median", "ksize": 5}}
param_background_removal = {{"method": "tophat", "wnd": 15}}
noise_freq = 0.01
sparse_penal = 0.01

param_pnr_refine = {{"noise_freq": noise_freq, "thres": {pnr_thresh} }}
param_ks_refine = {{"sig": 0.05}}
param_seeds_merge = {{"thres_dist": 10, "thres_corr": 0.8, "noise_freq": 0.01}}
param_initialize = {{"thres_corr": 0.8, "wnd": 10, "noise_freq": 0.01}}
param_init_merge = {{"thres_corr": 0.8}}

# CNMF Parameters#
param_get_noise = {{"noise_range": (0.01, 0.5)}}
param_first_spatial = {{
    "dl_wnd": 10,
    "sparse_penal": sparse_penal,
    "size_thres": (25, None),
}}

param_first_merge = {{"thres_corr": 0.8}}

minian_path = "/home/lq53/mir_repos/minian/minian"

dpath = os.path.dirname(pathlib.Path(__file__).resolve())
minian_ds_path = os.path.join(dpath, "minian")
intpath = os.path.join(dpath, "minian_intermediate")
nc_file_name = os.path.join(dpath, "minian_dataset_{unique_id}.nc")

subset = dict(frame=slice(0, None))
subset_mc = None
interactive = True
output_size = 100
n_workers = int(os.getenv("MINIAN_NWORKERS", 4))
param_save_minian = {{
    "dpath": minian_ds_path,
    "meta_dict": dict(session=-1, animal=-2),
    "overwrite": True,
}}

param_load_videos = {{
    "pattern": "[0-9]+\\.avi$",
    "dtype": np.uint8,
    "downsample": dict(frame=1, height=1, width=1),
    "downsample_strategy": "subset",
}}

subset_mc = None
param_estimate_motion = {{"dim": "frame"}}

os.environ["OMP_NUM_THREADS"] = "1"
os.environ["MKL_NUM_THREADS"] = "1"
os.environ["OPENBLAS_NUM_THREADS"] = "1"
os.environ["MINIAN_INTERMEDIATE"] = intpath
"""

    # === 网格搜索循环 ===
    for wnd_size, stp_size, max_wnd, diff_thres, pnr_thresh in itertools.product(
        wnd_size_vals, stp_size_vals, max_wnd_vals, diff_thres_vals, pnr_thresh_vals
    ):
        # 格式化 pnr_thresh
        if isinstance(pnr_thresh, str):
            pnr_thresh_str = f'"{pnr_thresh}"'
        else:
            pnr_thresh_str = pnr_thresh

        combination_str = f"wnd{wnd_size}_stp{stp_size}_max{max_wnd}_diff{diff_thres}_pnr{pnr_thresh}"
        output_nc = os.path.join(session_dir, f"minian_dataset_{combination_str}.nc")

        # —— 跳过条件 1：已在日志
        if combination_str in completed_combinations:
            print(f"[DryRun] Skipping (found in log): {combination_str}")
            continue

        # —— 跳过条件 2：输出文件已存在
        if os.path.exists(output_nc):
            print(f"[DryRun] Skipping (output exists): {combination_str}")
            completed_combinations.add(combination_str)
            continue

        # 如果是 dry_run，只打印“会运行哪个组合”，不实际执行
        if dry_run:
            print(f"[DryRun] Would run: {combination_str}")
            continue

        # 以下为非 dry_run 真正执行的部分
        print(f"Running combination: {combination_str}")

        # 生成 param 文件
        param_content = param_template.format(
            wnd_size=wnd_size,
            stp_size=stp_size,
            max_wnd=max_wnd,
            diff_thres=diff_thres,
            pnr_thresh=pnr_thresh_str,
            unique_id=combination_str
        )
        param_file_path = os.path.join(session_dir, "minian_param_mir.py")
        with open(param_file_path, "w") as f:
            f.write(param_content)

        # 真正调用 minian 脚本
        result = subprocess.run(
            ["python", "minian_mirpy_set.py"],
            cwd=session_dir,
            capture_output=True,
            text=True
        )
        if result.stdout:
            print(result.stdout)
        if result.stderr:
            print("Errors:", result.stderr)

        # 检查输出文件
        if not os.path.exists(output_nc):
            print(f"Warning: Output file not found for combination {combination_str}.")
            output_nc_record = "Not produced"
        else:
            output_nc_record = output_nc

        # 写入日志（追加模式）
        timestamp = datetime.now().isoformat()
        with open(log_file, "a", newline="") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow([
                timestamp,
                combination_str,
                wnd_size,
                stp_size,
                max_wnd,
                diff_thres,
                pnr_thresh,
                output_nc_record
            ])

        completed_combinations.add(combination_str)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run parameter search for a given session."
    )
    parser.add_argument(
        "--session_dir", required=True, help="Path to the session directory."
    )
    parser.add_argument(
        "--animal", default="UnknownAnimal", help="Animal ID (optional)."
    )
    parser.add_argument(
        "--dry_run", action="store_true",
        help="如果指定，则只打印会做哪些操作，不真正运行 minian 脚本，也不写日志。"
    )

    args = parser.parse_args()
    main(args.session_dir, args.animal, dry_run=args.dry_run)
