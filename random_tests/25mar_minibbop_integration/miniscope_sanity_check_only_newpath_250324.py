...
def load_manual_miniscope_paths(csv_file):
    """
    Load miniscope path entries from the manual CSV file.
    """
    manual_paths = set()
    if os.path.exists(csv_file):
        with open(csv_file, "r") as f:
            ...
            for row in reader:
                # Assuming the CSV has a column named 'rec_path' that actually contains the miniscope path.
                path = row.get("rec_path", "").strip()
                if path:
                    manual_paths.add(path)
    else:
        print(f"Manual CSV file not found: {csv_file}")
    return manual_paths

def main():
    # Path to the JSON mapping file.
    json_file = "./mini_to_rec_mapping.json"
    if not os.path.exists(json_file):
        print(f"JSON file not found: {json_file}")
        return

    # Path to the manual CSV file.
    manual_csv_file = "/home/lq53/mir_repos/BBOP/random_tests/25mar_minibbop_integration/mini_sanity_240324_mannual.csv"
    manual_miniscope_paths = load_manual_miniscope_paths(manual_csv_file)
    ...
    for mini_path, mapping in mapping_data.items():
        ...
        # Skip if the miniscope path (the JSON key) is already marked in the manual CSV file.
        if mini_path in manual_miniscope_paths:
            print(f"\nSkipping {mini_path} because it is marked in the manual CSV.")
            continue
