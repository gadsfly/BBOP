...
def main():
    driver_filename = "param_search_driver.py"
    with open(driver_filename, "w") as f:
        f.write(script_code)
    print(f"Created {driver_filename} with the parameter search code.")
    print("You can now run it via:")
    print(f"  python {driver_filename} --session_dir /path/to/session --animal SomeAnimal\n")
